...
class UnsplashCollections(UnsplashBase):
    ...
    def list_collection(self, page: int = 1, items_per_page: int = 10):
        ...
        response = self.session.get(endpoint,
                                    params=dict(client_id=self.access_key,
                                                page=page, order_by=self.order_by,
                                                per_page=items_per_page))
        if not response.status_code == 200:
            raise Exception(f'Not able to extract content. Code - {response.status_code}')
    ...
    def get_collection_photos_(self, collection_id, page_limit: int = 10, per_page: int = 10,
                               **kwargs):
        """
    ...
    def get_photo_statistics(self, collections_id):
        """
    ...
    def update_collection(self, collections_id, **kwargs)s:
        """
